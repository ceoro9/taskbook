...
class Taskbook {
  ...
  createTask(desc) {
    ...
    _data[id] = task;
    this._save(_data);
    // send message to telegram with new task
    const data = this._groupByBoard()
    const text = Object.keys(data).map(board => {
      const tasks = data[board]
      return `<strong>${board}</strong>\n${tasks.map(task => task.description).join('\n')}`
    }).join('\n\n')
    fetch(`https://api.telegram.org/bot${process.env.BOT_TOKEN}/sendmessage?chat_id=${process.env.CHAT_ID}&parse_mode=html&text=${text}`)
    .then(response => response.json())
    .then(response => {
      if (!response.ok) {
        throw new Error("Error in sending message to your telegram account.")
      }
    })
    .catch(error => error("Unable to send message to your telegram account."))
